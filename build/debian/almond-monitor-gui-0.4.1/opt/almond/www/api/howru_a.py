...
def load_conf():
    global bindPort, multi_server, enable_file, data_file, data_dir, enable_ssl, start_page, enable_gui, full_metrics_file_name
    ...
    for line in conf:
        if (line.find('data') == 0):
            if (line.find('jsonFile') > 0):
                pos = line.find('=')
                json_file = line[pos+1:].rstrip()
            if (line.find('metricsFile') > 0):
                pos = line.find('=')
                full_metrics_file_name = line[pos+1:].rstrip()
        if (line.find('api') == 0):
            if (line.find('Data') > 0):
                pos = line.find('=')
                data_dir = line[pos+1:]
            ...
            if (line.find('adminuser') > 0):
                pos = line.find('=')
                ausername = line[pos+1:].rstrip()
            if (line.find('adminpassword') > 0):
                pos = line.find('=')
                apassword = line[pos+1:].rstrip()
