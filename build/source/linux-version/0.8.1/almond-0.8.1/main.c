...
int logmessage_size = 1545;
int server_fd;
int pluginitemname_size = 50;
int pluginitemdesc_size = 100;
int pluginitemcmd_size = 255;
int pluginoutput_size = 1500;
int plugincommand_size = 100;
int dataFileNameSize = 100;
int newFileNameSize = 250;
int storedir_size = 50;
int backupDirectorySize = 100;
int filename_size = 100;
int logfile_size = 100;
int is_file_open = 0;
int declaration_size = 0;
int output_size = 0;
int update_output_size = 0;
int update_declaration_size = 0;
...
void writeLog(const char *message, int level, int startup) {
	...
	if (logmessage == NULL) {
        ...
        strncpy(logmessage, timeStamp, sizeof(timeStamp)-1);
	/*if (logmessage != NULL) {
		free(logmessage);
		logmessage = NULL;
...
void initConstants() {
	...
	logfile = malloc(logfile_size * sizeof(char));
	dataFileName = malloc(dataFileNameSize * sizeof(char));
	memset(dataFileName, 0, 100 * sizeof(char));
	backupDirectory = malloc(backupDirectorySize * sizeof(char));
	memset(backupDirectory, 0, 100 * sizeof(char));
	newFileName = malloc(newFileNameSize * sizeof(char));
	memset(newFileName, 0, 150 * sizeof(char));
	gardenerRetString = malloc(gardenermessage_size * sizeof(char));
	memset(gardenerRetString, 0, sizeof(char) * gardenermessage_size);
	...
	memset(pluginReturnString, 0, pluginmessage_size * sizeof(char));
	storeName = malloc(storename_size * sizeof(char));
	memset(storeName, 0, sizeof(char) * storename_size);
	...
	writeLog("Reading memory variable constants.", 0, 1);
        ...
        for (int i = 0; i < count; i++) {
                if (strcmp(constants[i], "CONFDIR_SIZE") == 0) {
                        ...
                        char *temp  = realloc(confDir, values[i] * sizeof(char)+1);
			...
			//char *temp = realloc(dataDir, values[i] * sizeof(char)+1);
			dataDir = realloc(dataDir, values[i] * sizeof(char)+1);
			if (dataDir == NULL) {
				...
				memset(dataDir, 0, values[i] * sizeof(char)+1);
			//dataDir = temp;
		}
			...
			pluginDeclarationFile = realloc(pluginDeclarationFile, values[i] * sizeof(char)+1);
			if (pluginDeclarationFile == NULL) {
				...
				memset(pluginDeclarationFile, 0, values[i] * sizeof(char)+1);
			...
			free(jsonFileName);
			jsonFileName = NULL;
			jsonFileName = malloc(values[i] * sizeof(char)+1);
			if (jsonFileName == NULL) {
				...
				memset(jsonFileName, 0, values[i] * sizeof(char)+1);
				strncpy(jsonFileName, "monitor_data.json", 18);
                        ...
                        metricsFileName = malloc(values[i] * sizeof(char)+1);
			if (metricsFileName == NULL) {
				...
				memset(metricsFileName, 0, values[i] * sizeof(char)+1);
                        ...
                        free(gardenerScript);
                        gardenerScript = NULL;
                        gardenerScript = malloc(values[i] * sizeof(char)+1);
			if (gardenerScript == NULL) {
				...
				memset(gardenerScript, 0, values[i] * sizeof(char)+1);
                        	strncpy(gardenerScript, "/opt/almond/gardener.py", 24); 
			}
		} 
			...
			writeLog("Memory for variable 'hostName' will be reallocated by constants file.", 0, 1);
                        free(hostName);
                        hostName = NULL;
                        hostName = malloc(values[i] * sizeof(char)+1);
			if (hostName == NULL) {
				...
				memset(hostName, 0, values[i] * sizeof(char)+1);
                        	strncpy(hostName, "None", 5);
			}
		}
                        ...
                        metricsOutputPrefix = malloc(values[i] * sizeof(char)+1);
                        if (metricsOutputPrefix == NULL) {
				...
				memset(metricsOutputPrefix, 0, values[i] * sizeof(char)+1);
                        	strncpy(metricsOutputPrefix, "almond", 7);
			}
                }
			...
			storeDir = realloc(storeDir, values[i] * sizeof(char)+1);
			storedir_size = values[i];
			if (storeDir == NULL) {
				...
				memset(storeDir, 0, values[i] * sizeof(char)+1);
                        	//storeDir = temp;
			}
                }
			...
			logDir = realloc(logDir, values[i] * sizeof(char)+1);
                        if (logDir == NULL) {
				...
				memset(logDir, 0, values[i] * sizeof(char)+1);
                        	//logDir = temp;
			}
                }
			...
			infostr = realloc(infostr, infostr_size * sizeof(char)+1);
                        if (infostr == NULL) {
				...
				memset(infostr, 0, values[i] * sizeof(char)+1);
			//infostr = temp;
		}
			...
			pluginDir = realloc(pluginDir, values[i] * sizeof(char)+1);
			if (pluginDir == NULL) {
				...
				memset(pluginDir, 0, values[i] * sizeof(char)+1);
                        //pluginDir = temp;
                }
			...
			fileName = realloc(fileName, values[i] * sizeof(char)+1);
                        if (fileName == NULL) {
				...
				memset(fileName, 0, values[i] * sizeof(char)+1);
			...
			logmessage = realloc(logmessage, values[i] * sizeof(char)+1);
                        if (logmessage == NULL) {
				...
				memset(logmessage, 0, values[i] * sizeof(char)+1);
			logmessage_size = values[i];
			logmessage[0] = '\0';
		}
			...
			logfile = realloc(logfile, values[i] * sizeof(char)+1);
                        if (logfile == NULL) {
				...
				memset(logfile, 0, values[i] * sizeof(char)+1);
			logfile_size = values[i];
		}
			...
			dataFileNameSize = values[i];
                        //char *temp = realloc(dataFileName, dataFileNameSize * sizeof(char)+1);
			dataFileName = realloc(dataFileName, dataFileNameSize * sizeof(char)+1);
                        if (dataFileName == NULL) {
				...
				memset(dataFileName, 0, values[i] * sizeof(char)+1);
			dataFileName[0] = '\0';
                }
			...
			backupDirectorySize = values[i];
                        //char *temp = realloc(backupDirectory, values[i] * sizeof(char)+1);
			backupDirectory = realloc(backupDirectory, values[i] * sizeof(char)+1);
                        if (backupDirectory == NULL) {
				...
				memset(backupDirectory, 0, values[i] * sizeof(char)+1);
                }
			...
			newFileName = realloc(newFileName, values[i] * sizeof(char)+1);
			newFileNameSize = values[i];
                        if (newFileName == NULL) {
				...
				memset(newFileName, 0, values[i] * sizeof(char)+1);
			newFileName[0] = '\0';
                }
			...
			if (gardenerRetString == NULL) {
				...
				memset(gardenerRetString, 0, values[i] * sizeof(char)+1);
		}
			...
			pluginCommand = realloc(pluginCommand, values[i] * sizeof(char)+1);
			plugincommand_size = values[i];
			if (pluginCommand == NULL) {
				...
				memset(pluginCommand, 0, values[i] * sizeof(char)+1);
		}
			...
			pluginReturnString = realloc(pluginReturnString, pluginmessage_size * sizeof(char)+1);
				...
				memset(pluginReturnString, 0, values[i] * sizeof(char)+1);
                }
			...
			storeName = realloc(storeName, storename_size * sizeof(char)+1);
				...
				memset(storeName, 0, values[i] * sizeof(char)+1);
                }
...
void send_socket_message(int socket, int id, int aflags) {
	...
	if (send_message == NULL) {
        ...
        strncpy(send_message, header, sizeof(header));
...
void collectMetrics(int decLen, int style) {
	...
	for (int i = 0; i < decLen; i++) {
		...
		if (strchr(outputs[i].retString, '|') == NULL) {
			...
			char * metrics = malloc(sizeof(char) * sublen);
			...
			if (style == 0)
				...
				while (token != NULL) {
					...
					metricsValue = malloc(metricsValueLength);
...
int redeclarePluginDeclarations(int mode, int count) {
	...
	update_outputs = (PluginOutput *)calloc(count, sizeof(PluginOutput));
	update_output_size = count;
	if (!update_outputs){
		perror("Error allocating memory");
	...
	switch(mode) {
		case 0:
			if (count > 0 && threadIds != NULL) {
				threadIds = realloc(threadIds, count * sizeof(int));
				if (threadIds == NULL) {
					free(threadIds);
...
int main() {
	...
	//initLogMessages();
	initConstants();
	writeLog("Starting almond (0.8.1)...", 0, 1);
