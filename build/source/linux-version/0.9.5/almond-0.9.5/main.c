...
void initConstants() {
	logmessage = calloc(logmessage_size, sizeof(char));
	if (logmessage == NULL) {
	...
	jsonFileName = calloc(jsonfilename_size, sizeof(char));
	if (jsonFileName == NULL) {
                fprintf(stderr, "Failed to allocate memory [jsonFileName].\n");
        }
	else
		strncpy(jsonFileName, "monitor_data.json", 18);
	metricsFileName = calloc(metricsfilename_size, sizeof(char));
	...
	gardenerScript = calloc(gardenerscript_size, sizeof(char));
	...
	hostName = calloc(hostname_size, sizeof(char));
	if (hostName == NULL) {
	...
	metricsOutputPrefix = calloc(metricsoutputprefix_size, sizeof(char));
...
int main() {
	...
	initConstants();
	writeLog("Starting almond (0.9.5)...", 0, 1);
	...
	if (check_plugin_conf_file(pluginDeclarationFile) != 0) {
                ...
                printf("Errors found in plugins.conf file\n");
                return 2;
        }
	...
	writeLog("Declarations read.", 0, 0);
