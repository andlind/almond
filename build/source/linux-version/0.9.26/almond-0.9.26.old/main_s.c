		...
		if (use_push) {
                        ...
                        if (push_interval_cnt >= sleep_push_interval) {
                                ...
                                if (url[0] == '\0') {
                                        ...
                                        snprintf(infostr, infostr_size, "Pushing data from file: %s", json_path);
                                        if (post_json_file_stream(url, json_path) == 0) {
                                                printf("DEBUG: File pushed successfully\n");
                                                writeLog("Data pushed successfully.", 0, 0);
                                        }
                                        else {
                                                printf("DEBUG: Failed to push file\n");
...
void initialLogging() {
	...
	if (!fptr) {
        ...
        writeLog("Starting almond (0.9.26)...", 0, 1);
}
