...
from flask import Flask, request, abort, jsonify, render_template, redirect, url_for, send_from_directory, make_response
from werkzeug.datastructures import MultiDict
from admin_page import admin_page
from converter.nagios_to_otel import nagios_to_otel
from converter.otel_exporter import export_otel_data
from converter.otel_exporter import CollectorConnectionError, set_otlp_endpoint
...
def load_conf():
    ...
    r_otlp_exporter = config.get('api.enableOtlpExporter', 'false')
...
def periodic_otlp_exporter():
    ...
    while True:
        try:
            checks = data["monitoring"]
            otel_objects = [nagios_to_otel(c) for c in checks]
            all_checks = extract_all_checks()
            if not all_checks:
                logger.warning("Could not extract all checks in periodic otlp exporter.")
                return
    ...
    while True:
        try:
            ...
            if mtime != last_mtime:
                ...
                otel_objects = [nagios_to_otel(check, host) for check, host in all_checks]
                export_otel_data(otel_objects)
                print("[File Watcher] Exported due to file change")
        except Exception as e:
