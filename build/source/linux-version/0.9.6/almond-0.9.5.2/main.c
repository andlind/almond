...
void runPlugin(int storeIndex, int update) {
	...
	if (update == 0) {
		if (outputs[storeIndex].prevRetCode != -1){
			...
			printf("DEBUG: scheduler[%d].id = %d, old timestamp is %ld\n", declarations[storeIndex].id-1, scheduler[declarations[storeIndex].id-1].id, scheduler[declarations[storeIndex].id-1].timestamp);
			if (timeScheduler == 1) {
				scheduler[declarations[storeIndex].id-1].timestamp = nextTime;
				printf("DEBUG: scheduler[%d].id = %d, new timestamp is %ld\n", declarations[storeIndex].id-1, scheduler[declarations[storeIndex].id-1].id, scheduler[declarations[storeIndex].id-1].timestamp);
			}
		}
		else {
...
void initialLogging() {
	char lfin[28] = "/var/log/almond/almond.log";
        ...
        writeLog("Starting almond (0.9.5.2)...", 0, 1);
}
