...
static int parseLine(const char *line, Declaration *decl, int id, int lineno) {
	char *cpy = strdup(line);
    	if (!cpy) {
        	writeLog("Memory allocation failure during line duplication", LOG_LEVEL_ERROR, 0);
        	return 0;
    	}
    	char *p = trim_line(cpy);

    	if (*p != '[') {
        	snprintf(infostr, sizeof(infostr),"Line %d: Missing '[' at beginning", lineno);
    	...
    	char *brkt = strchr(p, ']');
    	if (!brkt) {
        	snprintf(infostr, sizeof(infostr),"Line %d: Missing closing ']'", lineno);
        	writeLog(infostr, 1, 0);
        	free(cpy);
        	return 0;
    	}
    	*brkt = '\0';
    	char *name = trim_line(p + 1);

    	char *fields = trim_line(brkt + 1);
    	char *parts[4] = {0};
    	int idx = 0;
    	char *tok = strtok(fields, ";");
    	while (tok && idx < 4) {
        	parts[idx++] = trim_line(tok);
        	tok = strtok(NULL, ";");
    	}
    	if (idx != 4) {
        	snprintf(infostr, sizeof(infostr), "Line %d: Expected 4 fields, got %d", lineno, idx);
        	writeLog(infostr, 1, 0);
        	free(cpy);
        	return 0;
    	}
    	...
    	decl->description = strdup(parts[0]);
    	decl->command     = strdup(parts[1]);
    	decl->active      = atoi(parts[2]);
    	decl->interval    = atoi(parts[3]);
    	decl->id          = id;

    	if (!decl->name || !decl->description || !decl->command) {
        	writeLog("Memory allocation failure during strdup", LOG_LEVEL_ERROR, 0);
        	free(cpy);
        	return 0;
    	}
 	free(cpy);
...
void apiRunAndRead(int plugin_id, int flags) {
	...
	if (timeScheduler ==1)
		rescheduleChecks();
        createUpdateFile(&g_plugins[plugin_id], &outputs[plugin_id], strNum);
	strcpy(message, "{\n     \"executePlugin\":\"");
        strcat(message, pluginName);
        strcat(message, "\",\n");
        strcat(message, "      \"result\": {\n");
...
void collectMetrics(int decLen, int style) {
	...
	for (int i = 0; i < decLen; i++) {
		...
		//	writeLog(trim(infostr), 1, 0);
			...
			if (style == 0)
                       		fprintf(mf, "%s_%s{hostname=\"%s\",%s_result=\"%s\"} %d\n", prefix, pluginName, hostName, pluginName, trimmed_raw, outputs[i].retCode);
			else { 
				// Get service name	
				...
				fprintf(mf, "%s_%s{hostname=\"%s\", service=\"%s\", value=\"%s\"} %d\n", prefix, pluginName, hostName, service, trimmed_raw, outputs[i].retCode);
				free(serviceName);
				serviceName = NULL;
			}
		}
...
void writeToKafkaTopic(int storeIndex, int update) {
	...
	payload = malloc((size_t)count_bytes);
        ...
        if (kafka_export_addons < 1) {
        	sprintf(payload, "{\"name\":\"%s\", \"data\": {\"lastChange\":\"%s\", \"lastRun\":\"%s\", \"name\":\"%s\", \"nextRun\":\"%s\", \"pluginName\":\"%s\", \"pluginOutput\":\"%s\", \"pluginStatus\":\"%s\", \"pluginStatusChanged\":\"%s\", \"pluginStatusCode\":\"%d\"}}", hostName, g_plugins[storeIndex].lastChangeTimestamp, currTime, pluginName, g_plugins[storeIndex].nextRunTimestamp, g_plugins[storeIndex].description, outputs[storeIndex].retString, pluginStatus, g_plugins[storeIndex].statusChanged, outputs[storeIndex].retCode);
        	printf("Payload = %s\n", payload);
        }
        else {
       		if (kafka_export_addons == KAFKA_EXPORT_TAG) {
        		sprintf(payload, "{\"name\":\"%s\", \"tag\":\"%s\", \"data\": {\"lastChange\":\"%s\", \"lastRun\":\"%s\", \"name\":\"%s\", \"nextRun\":\"%s\", \"pluginName\":\"%s\", \"pluginOutput\":\"%s\", \"pluginStatus\":\"%s\", \"pluginStatusChanged\":\"%s\", \"pluginStatusCode\":\"%d\"}}", hostName, kafka_tag, g_plugins[storeIndex].lastChangeTimestamp, currTime, pluginName, g_plugins[storeIndex].nextRunTimestamp, g_plugins[storeIndex].description, outputs[storeIndex].retString, pluginStatus, g_plugins[storeIndex].statusChanged, outputs[storeIndex].retCode);
        	}
        	else {
        		...
        		if (kafka_export_addons == KAFKA_EXPORT_ID) {
        			sprintf(payload, "{\"name\":\"%s\", \"id\":\"%s\", \"data\": {\"lastChange\":\"%s\", \"lastRun\":\"%s\", \"name\":\"%s\", \"nextRun\":\"%s\", \"pluginName\":\"%s\", \"pluginOutput\":\"%s\", \"pluginStatus\":\"%s\", \"pluginStatusChanged\":\"%s\", \"pluginStatusCode\":\"%d\"}}", hostName, kafka_id, g_plugins[storeIndex].lastChangeTimestamp, currTime, pluginName, g_plugins[storeIndex].nextRunTimestamp, g_plugins[storeIndex].description, outputs[storeIndex].retString, pluginStatus, g_plugins[storeIndex].statusChanged, outputs[storeIndex].retCode);
        		}
...
void copyPluginItem(PluginItem *dest, const PluginItem *src, int mode) {
    ...
    if (mode == 0) {
        if (src->name != NULL) {
            snprintf(dest->lastRunTimestamp, 20, "%s", src->lastRunTimestamp);
            snprintf(dest->nextRunTimestamp, 20, "%s", src->nextRunTimestamp);
            snprintf(dest->lastChangeTimestamp, 20, "%s", src->lastChangeTimestamp);
...
int redeclarePluginDeclarations(int mode, int count) {
	...
	//destroy_declataions(update_g_plugins, count);
	update_g_plugins = (PluginItem *)calloc(count, sizeof(PluginItem));
	update_declaration_size = (size_t)count;
	if (!update_g_plugins) {
		perror ("Error allocating memory");
		writeLog("Error allocating memory [redeclarePluginDeclarations:update_g_plugins]", 2, 0);
		abort();
		return 2;
	}
	for (int i = 0; i < count; i++) {
		/*update_g_plugins[i].name = (char *) malloc((size_t)pluginitemname_size * sizeof(char));
		update_g_plugins[i].description = (char *) malloc((size_t)pluginitemdesc_size * sizeof(char));
		update_g_plugins[i].command = (char *) malloc((size_t)pluginitemcmd_size * sizeof(char));*/
		update_g_plugins[i].name = calloc(pluginitemname_size + 1, 1);
    		update_g_plugins[i].description = calloc(pluginitemdesc_size + 1, 1);
    		update_g_plugins[i].command = calloc(pluginitemcmd_size + 1, 1);
		if (update_g_plugins[i].name == NULL || update_g_plugins[i].description == NULL || update_g_plugins[i].command == NULL) {
	...
	for (size_t i = 0; i < update_output_size; ++i) {
    		plugin_output_init(&update_outputs[i]);
	}
	for (int j = 0; j < count; j++) {
		//update_outputs[j].retString = (char *) malloc((size_t)pluginoutput_size * sizeof(char));
		update_outputs[j].retString = calloc(pluginoutput_size + 1, 1);
	...
	if (pluginDeclarationResult <= 0){
		...
		printf("Loaded %d plugin g_plugins.\n", pluginDeclarationResult);
		writeLog("Plugin g_plugins file reloaded.", 0, 0);
	}
	int update[count];
	switch(mode) {
		case 0:
			...
			for (int i = 0; i < decCount; i++) {
                        ...
                        for (int i = 0; i < count; i++) {
                                if (update[i] == 0){
                                        copyPluginItem(&update_g_plugins[i], &g_plugins[i],0);

                                        //copyOutputItem(&update_outputs[i], &outputs[i]);
					int err = plugin_output_set(&update_outputs[i], &outputs[i]);
					if (err) {
						writeLog("[redeclarePluginOutput] plugin_output_set failed.", 1, 0);
					}
                                }
                                else {
                                        copyPluginItem(&update_g_plugins[i], &g_plugins[update[i]], 0);
					if (outputs[update[i]].retString != NULL) {
			...
			for (size_t i = 0; i < decCount; ++i) {
    				plugin_output_destroy(&outputs[i]);
                        ...
                        free(outputs);
			// Could we go just like this
			//outputs = update_outputs;
                        g_plugins = NULL;
                        g_plugins = (PluginItem *)malloc((size_t)sizeof(PluginItem) * count);
			...
			for (size_t i = 0; i < count; ++i) {
    				plugin_output_init(&outputs[i]);
                        ...
                        for (int i = 0; i < count; i++) {
                                /*g_plugins[i].name = (char *) malloc((size_t)pluginitemname_size * sizeof(char));
                                g_plugins[i].description = (char *) malloc((size_t)pluginitemdesc_size * sizeof(char));
                                g_plugins[i].command = (char *) malloc((size_t)pluginitemcmd_size * sizeof(char));*/
				...
				outputs[i].retString = calloc(pluginoutput_size + 1, 1);
                        ...
                        for (int e = 0; e < count; e++) {
                                copyPluginItem(&g_plugins[e], &update_g_plugins[e], 5);
                                //copyOutputItem(&outputs[e], &update_outputs[e]);
				int err = plugin_output_set(&outputs[e], &update_outputs[e]);
                                if (err) {
                                	writeLog("[redeclarePluginOutput] plugin_output_set failed.", 1, 0);
                      		}
                        }
					...
					// This is now freed by plugin_output_destroy!?!
                                        //free(update_outputs[i].retString);
                                }
                        }
                        free(update_g_plugins);
			for (size_t i = 0; i < update_output_size; ++i) {
    				plugin_output_destroy(&update_outputs[i]);
			}
                        free(update_outputs);
                        update_g_plugins = NULL;
                        update_outputs = NULL;
		...
		case 1:
			...
			for (int i = 0; i < count; i++) {
                                ...
                                for (int j = 0; j < decCount; j++) {
                                        if (strcmp(update_g_plugins[i].name, g_plugins[j].name) == 0) {
                                                ...
                                                if (j == decCount-1) {
                                                        ...
                                                        snprintf(infostr, infostr_size, "Old plugin declaration '%s' with id %d marked for deletion.", g_plugins[j].name, g_plugins[j].id);
                                                        writeLog(trim(infostr), 1, 0);
                                                }
                                        }
                                }
                        ...
                        for (int a = 0; a < count; a++) {
                                 if (update[a] == 0){
                                        copyPluginItem(&update_g_plugins[a], &g_plugins[a],0);
                                        //copyOutputItem(&update_outputs[a], &outputs[a]);
					int err = plugin_output_set(&update_outputs[a], &outputs[a]);
                                        if (err) {
                                        	writeLog("[redeclarePluginOutput] plugin_output_set failed.", 1, 0);
                                        }
                                }
                                else {
                                        copyPluginItem(&update_g_plugins[a], &g_plugins[update[a]], 0);
			...
			for (size_t i = 0; i < output_size; ++i) {
				...
				outputs[i].retString = calloc(pluginoutput_size +1, 1);
                        ...
                        for (int z = 0; z < count; z++) {
                                copyPluginItem(&g_plugins[z], &update_g_plugins[z], 3);
                                //copyOutputItem(&outputs[z], &update_outputs[z]);
				int err = plugin_output_set(&update_outputs[z], &outputs[z]);
                                if (err) { 
                                	writeLog("[redeclarePluginOutput] plugin_output_set failed.", 1, 0);
                      		}
                        }
			for (int i = 0; i < count; i++) {
                                ...
                                free(update_g_plugins[i].command);
                                if (i < decCount) {
					// This should be removed by plugin_output_destroy
                                        //free(update_outputs[i].retString);
                                }
                        }
                        free(update_g_plugins);
		...
		case 2:
			c = 0;
                        for (int i = 0; i < decCount; i++) {
				if (strcmp(update_g_plugins[i].name, g_plugins[i].name) != 0) {
					c++;
					rows++;
					if (strcmp(update_g_plugins[i].description, g_plugins[i].description) == 0)
						c--;
					else
						c++;
					if (strcmp(update_g_plugins[i].command, g_plugins[i].command) == 0) 
						c--;
					else 
						c++;
				}
		...
		case 3:
			for (int i = 0; i < decCount; i++) {
				...
				for (int j = 0; j < decCount; j++) {
					...
					if (strcmp(update_g_plugins[i].description, g_plugins[j].description) == 0) {
						found++;
						id = j;
					}
					if (strcmp(update_g_plugins[i].command, g_plugins[j].command) == 0) {
						found++;
						id = j;
					}
				}
				if (found > 0) {
					...
					int err = plugin_output_set(&update_outputs[i], &outputs[id]);
    					if (err) {
						writeLog("[redeclarePluginDeclarations] Mode 3 plugin_output_set failed.", 1, 0);
    					}
					copyPluginItem(&update_g_plugins[i], &g_plugins[id], 2);
				}
				else {
					printf("Did not find declaration.name = %s", update_g_plugins[i].name);
					initNewPlugin(i);
				}
			}
			free_structures(decCount);
			...
			for (size_t i = 0; i < output_size ; ++i) {
				...
				outputs[i].retString = calloc(pluginoutput_size + 1, 1);
                                ...
                                copyPluginItem(&g_plugins[i], &update_g_plugins[i], 0);
                                //copyOutputItem(&outputs[i], &update_outputs[i]);
				int err = plugin_output_set(&outputs[i], &update_outputs[i]);
				if (err) {
					writeLog("[redeclarePluginDeclarations] Mode 3 plugin_output_set failed.", 1, 0);
				}
                        }
			...
			// This case in not working. Make corrupt data. Force restart?
                        /*for (int i = 0; i < decCount; i++) {
                                ...
                                for (int j = 0; j < count; j++) {
                                        printf ("j = %d\n", j);
                                        printf ("update_i = %s\n", update_g_plugins[i].name);
                                        printf ("dec_j = %s | outputs[%d] = %s\n", g_plugins[j].name, j, outputs[j].retString);
                                        if (strcmp(update_g_plugins[i].name, g_plugins[j].name) == 0) {
                                                printf("NAME\n");
                                                printf("Update_g_plugins = %s | g_plugins = %s\n", update_g_plugins[i].name, g_plugins[j].name);
                                                id = j;
                                                printf("ID = %d\n", id);
                                                write_this++;
                                                break;
                                        }
                                        else if (strcmp(update_g_plugins[i].description, g_plugins[j].description) == 0) {
                                                id = j;
                                                printf("DESCRIPTION\n");
                                                printf("Update_g_plugins = %s | g_plugins = %s\n", update_g_plugins[i].description, g_plugins[j].description);
                                                printf("ID = %d\n", id);
                                                write_this++;
                                                break;
                                        }
                                        else if (strcmp(update_g_plugins[i].command,g_plugins[j].command) == 0) {
                                                id = j;
                                                printf ("COMMAND\n");
                                                printf("Update_g_plugins = %s | g_plugins = %s\n", update_g_plugins[i].command, g_plugins[j].command);
                                                printf("ID = %d\n", id);
                                                write_this++;
                                                break;
                                        }
                                ...
                                if (write_this == 0) {
                                        ...
                                        snprintf(loginfo, 400, "Found old outputs for plugin declaration '%s'.", update_g_plugins[i].name);
                                        writeLog(trim(loginfo), 0);
                                        flushLog();
                                        printf ("ID = %d\n", id);
                                        printf ("Update_g_plugins = %s | g_plugins = %s\n", update_g_plugins[i].name, g_plugins[id].name);
                                        printf ("Outputs[%d] = ", id);
                                        printf ("%s\n", outputs[id].retString);
                                        update_outputs[i] = outputs[id];
                                        strcpy(update_g_plugins[i].lastRunTimestamp,g_plugins[id].lastRunTimestamp);
                                        strcpy(update_g_plugins[i].nextRunTimestamp,g_plugins[id].nextRunTimestamp);
                                        strcpy(update_g_plugins[i].statusChanged,g_plugins[id].statusChanged);
                                        update_g_plugins[i].nextRun = g_plugins[id].nextRun;
                                }
...
int updatePluginDeclarationsOld() {
	FILE *fp = NULL;
	char* line = NULL;
	char* name = NULL;
        char *token = NULL;
	...
	if (newCount > decCount) {
		...
		// Read plugin g_plugins file and update g_plugins	
		...
		fp = fopen(pluginDeclarationFile, "r");
       	 	if (fp == NULL)
        	{
                	perror("Error while opening the plugin g_plugins file.\n");
                	writeLog("Error opening the plugin g_plugins file.", 2, 0);
			free(item.name);
                	free(item.description);
                	free(item.command);
        	...
        	while ((read = getline(&line, &len, fp)) != -1) {
				...
				if (strcmp(g_plugins[counter].name, item.name) != 0) {
					snprintf(infostr, infostr_size, "Declaration name for item %i changed from %s to %s.", counter, g_plugins[counter].name, item.name);
					//strncpy(g_plugins[counter].name, item.name, strlen(item.name) + 1);
					snprintf(g_plugins[counter].name, pluginitemname_size, "%s", item.name);
					reload_required = 1;
                                        writeLog(trim(infostr), 0, 0);
				}
				if (strcmp(g_plugins[counter].description, item.description) != 0) {
					snprintf(infostr, infostr_size, "Declaration description for %s changed from %s to %s.", g_plugins[counter].name, g_plugins[counter].description, item.description);
					//strncpy(g_plugins[counter].description, item.description, strlen(item.description) + 1);
					snprintf(g_plugins[counter].description, pluginitemdesc_size, "%s", item.description);
                                        writeLog(trim(infostr), 0, 0);
				}
				if (strcmp(g_plugins[counter].command, item.command) != 0) {
					snprintf(infostr, infostr_size, "Declaration command for %s changed to %s.", g_plugins[counter].name, item.command);
					//strncpy(g_plugins[counter].command, item.command, sizeof(g_plugins[counter].command -1));
					snprintf(g_plugins[counter].command, pluginitemcmd_size, "%s", item.command);
					g_plugins[counter].command[sizeof(g_plugins[counter].command -1)] = '\0';
					writeLog(trim(infostr), 0, 0);
				}
				if (g_plugins[counter].active != item.active) {
					if (item.active == 0) {
						snprintf(infostr, infostr_size, "Declaration %s is now inactive.", g_plugins[counter].name);
						g_plugins[counter].active = 0;
					}
					else {
						snprintf(infostr, infostr_size, "Declaration %s is now active", g_plugins[counter].name);
						g_plugins[counter].active = 1;
					}
					writeLog(trim(infostr), 0, 0);
				}
				if (g_plugins[counter].interval != item.interval) {
					snprintf(infostr, infostr_size, "Declaration %s interval changed from %i to %i.", g_plugins[counter].name, g_plugins[counter].interval, item.interval);
					g_plugins[counter].interval = item.interval;
					writeLog(trim(infostr), 0, 0);
				}
				/* int x = ((g_plugins[counter].command == item.command) && (g_plugins[counter].active == item.active) &&
						(g_plugins[counter].interval == item.interval)) ? 1 : 0;
				if (x != 0) {
					strncpy(g_plugins[counter].command, item.command, strlen(item.command) + 1);
					g_plugins[counter].active = item.active;
					g_plugins[counter].interval = item.interval;
...
void initScheduler(int numOfP, int msSleep) {
	...
	for (int i = 0; i < numOfP; i++)
	{
		if (g_plugins[i].active == 1)
		{
			...
			//strncpy(g_plugins[i].statusChanged, "0", 2);
...
void initialLogging() {
	char lfin[28] = "/var/log/almond/almond.log";
        ...
        writeLog("Starting almond (0.9.13)...", 0, 1);
}
...
int main(int argc, char* argv[]) {
	...
	if (loadPlugins() != 0) {
		logError("Failed to load plugin g_plugins", 2, 0);
		flushLog();
		return 2;
	}
	flushLog();
