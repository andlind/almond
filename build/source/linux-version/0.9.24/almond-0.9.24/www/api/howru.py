...
def load_conf():
    ...
    global ssl_certificate, run_with_wsgi, ssl_key, enable_scraper, mods_list, admin_auth_type, is_container, persistant_2fa, show_dashboard
    config = {}
...
def api_show_plugin(search=0, id=-1):
    ...
    if (do_start):
       ...
       for i in obj:
           if (search == 0):
               if ((id_count == id) or (name == i['pluginName']) or (index_name == i['name'])):
                   results.append(i)
                   break
           else:
               if ((name in i['name']) or (name in i['pluginName']) or (index_name in i['name'])):
                   results.append(i)
               elif (id_count == id):
                   results.append(i)
                   break
