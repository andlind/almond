...
void parseClientMessage(char str[], int arr[]) {
	...
	if (jname != NULL) {
        ...
        if ((strcmp(trim(action), "read") == 0) || (strcmp(trim(action), "get") == 0)) {
                //printf("You want to read a object.\n");
		if (aflags == 10) {
			api_action = API_READ_ALL;
                ...
                //printf("You want to execute an object.\n");
                api_action = API_RUN;
        }
...
int redeclarePluginDeclarations(int mode, int count) {
	...
	update_declarations = malloc(sizeof(struct PluginItem) * count);
	...
	update_outputs = malloc(sizeof(struct PluginOutput) * count);
	if (!update_outputs){
		perror("Error allocating memory");
