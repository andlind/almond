...
char constantsFile[26] = "/opt/almond/constants.alm";
char* confDir;
char* dataDir;
char* storeDir;
char* pluginDir;
...
int standalone = 0;
int quick_start = 0;
int infostr_size = 400;
int gardenermessage_size = 1035;
int pluginmessage_size = 2300;
int storename_size = 100;
int apimessage_size = 2000;
int socketservermessage_size = 2000;
int socketclientmessage_size = 2000;
int server_fd;
int pluginitemname_size = 50;
int pluginitemdesc_size = 100;
int pluginitemcmd_size = 255;
int pluginoutput_size = 1500;
...
void initConstants() {
	printf ("Initiating variable constants.\n");
	confDir = malloc(50 * sizeof(char));
	dataDir = malloc(50 * sizeof(char));
	pluginDir = malloc(50 * sizeof(char));
	pluginDeclarationFile = malloc(75 * sizeof(char));
	jsonFileName = malloc(50 * sizeof(char));
	if (jsonFileName == NULL) {
                fprintf(stderr, "Failed to allocate memory [jsonFileName].\n");
        }
	else
		strcpy(jsonFileName, "monitor_data.json");
	metricsFileName = malloc(50 * sizeof(char));
	if (metricsFileName == NULL) {
		fprintf(stderr, "Failed to allocate memory [metricsFileName].\n");
	}
	else
		strcpy(metricsFileName, "monitor.metrics");
	gardenerScript = malloc(75 * sizeof(char));
	if (gardenerScript == NULL) {
                fprintf(stderr, "Failed to allocate memory [gardenerScript].\n");
        }
        else
		strcpy(gardenerScript, "/opt/almond/gardener.py");
	storeDir = malloc(50 * sizeof(char));
	logDir = malloc(50 * sizeof(char));
	infostr = malloc(infostr_size * sizeof(char));
	...
	if (logmessage == NULL) {
		...
		strcpy(logmessage, "");
	logfile = malloc(100 * sizeof(char));
	dataFileName = malloc(100 * sizeof(char));
	backupDirectory = malloc(100 * sizeof(char));
	newFileName = malloc(150 * sizeof(char));
	gardenerRetString = malloc(gardenermessage_size * sizeof(char));
	pluginCommand = malloc(100 * sizeof(char));
	pluginReturnString = malloc(pluginmessage_size * sizeof(char));
	storeName = malloc(storename_size * sizeof(char));
...
int getConstants() {
	...
	writeLog("Reading variable constants.", 0);
        ...
        for (int i = 0; i < count; i++) {
                if (strcmp(constants[i], "CONFDIR_SIZE") == 0) {
                        writeLog("Memory for variable 'confDir' will be reallocated by constants file.", 0);
                        char *temp  = realloc(confDir, values[i] * sizeof(char));
			if (temp == NULL) {
				writeLog("Failed to reallocate memory for variable 'confDir'.", 1);
				return 1;
			}
			confDir = temp;
                }
		else if (strcmp(constants[i], "DATADIR_SIZE") == 0) {
			writeLog("Memory for variable 'dataDir' will be reallocated by constants file.", 0);
			char *temp = realloc(dataDir, values[i] * sizeof(char));
			if (temp == NULL) {
				writeLog("Failed to reallocate memory for variable 'dataDir'.", 1);
				return 1;
			}
			dataDir = temp;
		}
		else if (strcmp(constants[i], "PLUGINDECLARATIONFILE_SIZE") == 0) {
			writeLog("Memory for variable 'pluginDeclarationSize' will be reallocated by constants file.", 0);
			char *temp = realloc(pluginDeclarationFile, values[i] * sizeof(char));
			if (temp == NULL) {
				writeLog("Failed to reallocate memory for variable 'pluginDeclarationFile'.", 1);
				return 1;
			}
			pluginDeclarationFile = temp;
		}
		else if (strcmp(constants[i], "JSONFILENAME_SIZE") == 0) {
			writeLog("Memory for variable 'jsonFileName' will be reallocated by constants file.", 0);
			free(jsonFileName);
			jsonFileName = NULL;
			jsonFileName = malloc(values[i] * sizeof(char));
			if (jsonFileName == NULL) {
				writeLog("Failed to reallocate memory for variable 'jsonFileName'.", 1);
				return 1;
			}
			else
				strcpy(jsonFileName, "monitor_data.json");
                        ...
                        writeLog("Memory for variable 'metricsFileName' will be reallocated by constants file.", 0);
                        free(metricsFileName);
                        metricsFileName = NULL;
                        metricsFileName = malloc(values[i] * sizeof(char));
			if (metricsFileName == NULL) {
                                writeLog("Failed to reallocate memory for variable 'metricsFileName'.", 1);
                                return 1;
                        }
                        else
                        	strcpy(metricsFileName, "monitor_data.json");
                }
		else if (strcmp(constants[i], "GARDENERSCRIPT_SIZE") == 0) {
                        writeLog("Memory for variable 'gardenerScript' will be reallocated by constants file.", 0);
                        free(gardenerScript);
                        gardenerScript = NULL;
                        gardenerScript = malloc(values[i] * sizeof(char));
			if (gardenerScript == NULL) {
                                writeLog("Failed to reallocate memory for variable 'gardenerScript'.", 1);
                                return 1;
                        }
                        else
                        	strcpy(gardenerScript, "/opt/almond/gardener.py"); 
			...
			writeLog("Memory for variable 'hostName' will be reallocated by constants file.", 0);
                        free(hostName);
                        hostName = NULL;
                        hostName = malloc(values[i] * sizeof(char));
			if (hostName == NULL) {
				writeLog("Failed to reallocate memory for variable 'hostName'.", 1);
				return 1;
			}
			else
                        	strcpy(hostName, "None");
                        ...
                        writeLog("Memory for variable 'metricsOutputPrefix' will be reallocated by constants file.", 0);
                        free(metricsOutputPrefix);
                        metricsOutputPrefix = NULL;
                        metricsOutputPrefix = malloc(values[i] * sizeof(char));
                        if (metricsOutputPrefix == NULL) {
                                writeLog("Failed to reallocate memory for variable 'metricsOutputPrefix'.", 1);
                                return 1;
                        }       
                        else
                        	strcpy(metricsOutputPrefix, "almond");
                        ...
                        writeLog("Memory for variable 'storeDir' will be reallocated by constants file.", 0);
			char *temp = realloc(storeDir, values[i] * sizeof(char));
			if (temp == NULL) {
				writeLog("Failed to reallocate memory for variable 'storeDir'.", 1);
				return 1;
			}
                        storeDir = temp;
                }
		else if (strcmp(constants[i], "LOGDIR_SIZE") == 0) {
                        writeLog("Memory for variable 'logDir' will be reallocated by constants file.", 0);
                        char *temp = realloc(logDir, values[i] * sizeof(char));
                        if (temp == NULL) {
                                writeLog("Failed to reallocate memory for variable 'logDir'.", 1);
                                return 1;
                        }
                        logDir = temp;
                }
		else if (strcmp(constants[i], "INFOSTR_SIZE") == 0) {
			writeLog("Memory for 'info_str' will be reallocated by constants file.", 0);
			infostr_size = values[i];
                     	char *temp = realloc(infostr, infostr_size * sizeof(char));
                        if (temp == NULL) {
                                writeLog("Failed to reallocate memory for variable 'infostr'.", 1);
                                return 1;
                        }
			...
			infostr = temp;
                        ...
                        writeLog("Memory for variable 'pluginDir' will be reallocated by constants file.", 0);
			char *temp = realloc(pluginDir, values[i] * sizeof(char));
			if (temp == NULL) {
				writeLog("Failed to reallocate memory for variable 'pluginDir'.", 1);
				return 1;
			}
                        pluginDir = temp;
                }
		else if (strcmp(constants[i], "FILENAME_SIZE") == 0) {
                        writeLog("Memory for variable 'fileName' will be reallocated by constants file.", 0);
                        char *temp = realloc(fileName, values[i] * sizeof(char));
                        if (temp == NULL) {
                                writeLog("Failed to reallocate memory for variable 'fileName'.", 1);
                                return 1;
                        }
                        fileName = temp;
			...
			writeLog("Memory for variable 'logmessage' will be reallocated by constants file.", 0);
                        char *temp = realloc(logmessage, values[i] * sizeof(char));
                        if (temp == NULL) {
                                writeLog("Failed to reallocate memory for variable 'logmessage'.", 1);
				return 1;
			}
                        logmessage = temp;
			strcpy(logmessage, "");
			...
			writeLog("Memory for variable 'logfile' will be reallocated by constants file.", 0);
			char *temp = realloc(logfile, values[i] * sizeof(char));
                        if (temp == NULL) {
                                writeLog("Failed to reallocate memory for variable 'logfile'.", 1);
                                return 1;
                        }
			logfile = temp;
		}
		else if (strcmp(constants[i], "DATAFILENAME_SIZE") == 0) {
                        writeLog("Memory for variable 'dataFileName' will be reallocated by constants file.", 0);
                        char *temp = realloc(dataFileName, values[i] * sizeof(char));
                        if (temp == NULL) {
                                writeLog("Failed to reallocate memory for variable 'dataFileName'.", 1);
                                return 1;
                        }
                        dataFileName = temp;
                }
		else if (strcmp(constants[i], "BACKUPDIRECTORY_SIZE") == 0) {
                        writeLog("Memory for variable 'backupDirectory' will be reallocated by constants file.", 0);
                        char *temp = realloc(backupDirectory, values[i] * sizeof(char));
                        if (temp == NULL) {
                                writeLog("Failed to reallocate memory for variable 'backupDirecory'.", 1);
                                return 1;
                        }
                        backupDirectory = temp;
                }
		else if (strcmp(constants[i], "NEWFILENAME_SIZE") == 0) {
                        writeLog("Memory for variable 'newFileName' will be reallocated by constants file.", 0);
                        char *temp = realloc(newFileName, values[i] * sizeof(char));
                        if (temp == NULL) {
                                writeLog("Failed to reallocate memory for variable 'newFileNamee'.", 1);
                                return 1;
                        }
                        newFileName = temp;
                }
		else if (strcmp(constants[i], "GARDENERMESSAGE_SIZE") == 0) {
			writeLog("Memory for gardener return message will be reallocated by constants file.", 0);
			gardenermessage_size = values[i];
			char *temp = realloc(gardenerRetString, gardenermessage_size * sizeof(char));
			if (temp == NULL) {
				writeLog("Fauled to reallocate memory for gardener script return message.", 1);
				return 1;
			}
			gardenerRetString = temp;
		}
		else if (strcmp(constants[i], "PLUGINCOMMAND_SIZE") == 0) {
			writeLog("Memory for plugin command size will be reallocated by constants file.", 0);
			char *temp = realloc(pluginCommand, values[i] * sizeof(char));
			if (temp == NULL) {
				writeLog("Failed to reallocate memory for variable 'pluginCommand'.", 1);
				return 1;
			}
			pluginCommand = temp;
		}
		else if (strcmp(constants[i], "PLUGINMESSAGE_SIZE") == 0) {
                        writeLog("Memory for plugin message size will be reallocated by constants file.", 0);
			pluginmessage_size = values[i];
                        char *temp = realloc(pluginReturnString, pluginmessage_size * sizeof(char));
                        if (temp == NULL) {
                                writeLog("Failed to reallocate memory for variable 'pluginReturnString'.", 1);
                                return 1;
                        }
                        pluginReturnString = temp;
                }
		else if (strcmp(constants[i], "STORENAME_SIZE") == 0) {
                        writeLog("Memory for variable 'storeName' will be reallocated by constants file.", 0);
                        storename_size = values[i];
                        char *temp = realloc(storeName, storename_size * sizeof(char));
                        if (temp == NULL) {
                                writeLog("Failed to reallocate memory for variable 'storeName'.", 1);
                                return 1;
                        }
                        storeName = temp;
                }
		else if (strcmp(constants[i], "APIMESSAGE_SIZE") == 0) {
                        writeLog("Memory for API messages will dynamically be allocated by size inconstants file.", 0);
                        apimessage_size = values[i];
			...
			writeLog("Memory for socket server messages will dynamically be allocated by size in constants file.", 0);
			socketservermessage_size = values[i];
		}
		else if (strcmp(constants[i], "SOCKETCLIENTMESSAGE_SIZE") == 0) {
			writeLog("Memory for socket client messages will dynamically be allocated by size in constants file.", 0);
			socketclientmessage_size = values[i];
		}
		else if (strcmp(constants[i], "PLUGINITEMNAME_SIZE") == 0) {
			writeLog("Memory for pluginitem name size will dynamically be allocated by size in constants file.", 0);
                        pluginitemname_size = values[i];
                }
 		else if (strcmp(constants[i], "PLUGINITEMDESC_SIZE") == 0) {
                        writeLog("Memory for pluginitem description size will dynamically be allocated by size in constants file.", 0);
                        pluginitemname_size = values[i];
                }
 		else if (strcmp(constants[i], "PLUGINITEMCMD_SIZE") == 0) {
                        writeLog("Memory for pluginitem command size will dynamically be allocated by size in constants file.", 0);
                        pluginitemname_size = values[i];
                }
		else if (strcmp(constants[i], "PLUGINOUTPUT_SIZE") == 0) {
                        writeLog("Memory for plugin output will dynamically be allocated by size in constants file.", 0);
                        pluginitemname_size = values[i];
                }
		else {
			snprintf(infostr, infostr_size, "Constant '%s' not implemented by Almond %s", constants[i], VERSION);
			writeLog(trim(infostr), 1);
		}
        }
        return 0;
}
...
void constructSocketMessage(const char* action, const char* message) {
	...
	size += 11;
	socket_message = malloc(size);
    	if (socket_message == NULL) {
        	printf("Memory allocation failed.\n");
		writeLog("Memory allocation failed [constructSocketMessage:socket_message]", 2);
        	return;
    	}
    	snprintf(socket_message, size, "{ \"%s\":\"%s\" }\n", action, message);
...
int getIdFromName(char *plugin_name) {
	...
	for (int i = 0; i < decCount; i++) {
		...
		pluginName = malloc(pluginitemname_size * sizeof(char)+1);
		if (pluginName == NULL) {
			fprintf(stderr, "Failed to allocate memory.\n");
			writeLog("Failed to allocate memory [getIdFromName:pluginName]", 2);
			return -1;
		}
...
void changeSetValue(int id, int newval) {
	...
	switch (id) {
		...
		case 3:
				...
				writeLog("API call is trying to set sleep to unsupported value.", 1);
				writeLog("Scheduler sleep value is unchanged.", 0);
			}
			else
		...
		case 4:
			...
			break;
		default:
			writeLog("changeSetValue called with wrong index", 1);
	}
}
...
int toggleQuickStart(int on) {
	...
	if (fPtr == NULL || fTemp == NULL) {
		writeLog("Could not update quick start value in configuration file. Read error.", 1);
		exit(EXIT_SUCCESS);
	}
	...
	writeLog("Updated almond.conf file", 0);
	return 0;
}
...
void send_socket_message(int socket, int id, int aflags) {
	...
	if (send_message == NULL) {
		...
		writeLog("Could not allocate memory [send_socket_message:send_message]", 2);
		return;
	}
...
void parseClientMessage(char str[], int arr[]) {
	...
	args_set = 0;
		...
		writeLog(trim(infostr), 1);
		writeLog("Could not parse API call. Wrong syntax.", 1);
                return;
        }
	...
	if (jtoken != NULL) {
		token = malloc(30);
		if (token == NULL) {
			writeLog("Could not allocate memory for execute token", 1);
		}
		...
		trim(token);
                ...
                if (in_file == NULL)
                {
					...
					if (fname == NULL) {
						writeLog("Could not allocate message [parseClientMessage:fname]", 2);
						return;
					}
					strcpy(fname, trim(line));
                                }
                                if (i == 2){
                                        lname = malloc(sizeof(line)+1);
					if (lname == NULL) {
						writeLog("Could not allocate memory [parseClientMessage:lname]", 2);
						return;
					}
					strcpy(lname, trim(line));
                                }
                                i++;
		...
		if (bExecute != 0) {
			if (strcmp(trim(name), "pluginoutput") == 0) {
				...
				free(kafka_tag);
				kafka_tag = NULL;
                  		kafka_tag = malloc(strlen(sval)+1);
                  		strcpy(kafka_tag, trim(sval));
                  		snprintf(infostr, infostr_size, "Kafka tag is set to '%s'", kafka_tag);
				writeLog("Kafka tag is toggled through API call.", 0);
                  		writeLog(trim(infostr), 0);
				...
				if (val > 0) {
					changeSetValue(4, val);
                                	...
                                	writeLog("Kafka start id is toggled through API call.", 0);
                                	writeLog(trim(infostr), 0);
				}
				else {
					snprintf(infostr, infostr_size, "Could not set Kafka start id to '%s'", sval);
					writeLog("Kafka start id was toggled through API call.", 0);
					writeLog(trim(infostr), 1);
				}
				api_action = API_SET_KAFKATAG;
                        }
				...
				writeLog("Hostname (virtual) is toggled through API call.", 1);
				writeLog(trim(infostr), 1);
			...
			writeLog("API action was denied. Wrong or no token supplied.", 1);
			api_action = API_DENIED;
		}
	}
        else {
        ...
        if (api_action > 0) {
			...
			if (jname != NULL) {
				...
				writeLog("Received a bad json-request. API call is aborted.", 1);
				api_action = 0;
                        	return;
			}
			if (id < 0) {
				writeLog("Could not get id from name. This might cause strange things to happen. Aborting API call.", 1);
				api_action = 0;
				return;
			}
                }
		...
		if (args_set > 0 && (api_action == API_RUN || api_action == API_DRY_RUN || api_action == API_EXECUTE_AND_READ)) {
			...
			if (api_args == NULL) {
				fprintf(stderr, "Could not allocate memory.\n");
				writeLog("Could not allocate memory [parseClientMessage:api_args]", 2);
				return;
			}
...
int createSocket(int server_fd) {
	...
	server_message = malloc(socketservermessage_size);
	if (server_message == NULL) {
		fprintf(stderr, "Failed to allocate memory for servermessage.\n");
		writeLog("Failed to allocate memory [createSocket:servermessage].", 1);
		return -1;
	}
	client_message = malloc(socketclientmessage_size);
	if (client_message == NULL) {
		fprintf(stderr, "Failed to allocate memory for clientmessage.\n");
                writeLog("Failed to allocate memory [createSocket:clientmessage].", 1);
                return -1;
	}
        memset(server_message, '\0', socketservermessage_size);
        memset(client_message, '\0', socketclientmessage_size);
...
void free_constants() {
	...
	free(threadIds);
	//free(message);
	writeLog("All constants freed from memory.", 0);
}
...
void sig_handler(int signal){
    	switch (signal) {
			...
			free_kafka_vars();
			free_constants();
			writeLog("Almond says goodbye.", 0);
			fclose(fptr);
			free(logmessage);
...
int getConfigurationValues() {
	...
	while ((read = getline(&line, &len, fp)) != -1) {
	   ...
	   if (strcmp(confName, "scheduler.storeDir") == 0) {
                   if (directoryExists(confValue, 255) == 0) {
			   strcpy(storeDir, trim(confValue));
                           storeDirSet = 1;
                   }
                   else {
                           int status = mkdir(trim(confValue), 0755);
	   ...
	   if (strcmp(confName, "scheduler.enableClearDataCache") == 0) {
                   if (atoi(confValue) == 0) {
                           writeLog("Clear data cache is not enabled.", 0);
                   }
                   else {
                           writeLog("Clear data cache is enabled.", 0);
                           enableClearDataCache = 1;
                   }
           }
	   ...
	   if (strcmp(confName, "scheduler.enableKafkaId") == 0) {
                   if (atoi(confValue) == 0) {
                           writeLog("Use of Kafka id is not enabled.", 0);
                   }
                   else {
                           writeLog("Use of Kafka id is enabled.", 0);
                           enableKafkaId = 1;
                  }
           }
	   if (strcmp(confName, "scheduler.kafkaStartId") == 0) {
		   ...
		   if (i > 0) {
			   kafka_start_id = i;
			   writeLog("Kafka start id check ok", 0);
		   }
		   else {
			   writeLog("Could not read kafka_start_id.", 1);
			   kafka_start_id = 0;
		   }
	   }
	   if (strcmp(confName, "scheduler.kafkaBrokers") == 0) {
		   ...
		   if (kafka_brokers == NULL) {
			   ...
			   writeLog("Failed to allocate memory [kafka_brokers]", 2);
			   return 2;
		   }
	   ...
	   if (strcmp(confName, "scheduler.kafkaTopic") == 0) {
		   ...
		   if (kafka_topic == NULL) {
			   ...
			   writeLog("Failed to allocate memory [kafka_topic]", 2);
			   return 2;
		   }
		  ...
		  if (kafka_tag == NULL) {
			...
			writeLog("Failed to allocate memory [kafka_tag]", 2);
			return 2;
		  }
                  strcpy(kafka_tag, trim(confValue));
                  snprintf(infostr, infostr_size, "Kafka tag is set to '%s'", kafka_tag);
                  writeLog(trim(infostr), 0);
           }
	   if (strcmp(confName, "scheduler.enableKafkaSSL") == 0) {
		   if (atoi(confValue) == 0) {
			   writeLog("Kafka producer will connect with plain text", 0);
                           ...
                           writeLog("Kafka producer will connect to cluster with SSL.", 0);
                           writeLog("Make sure you use a certificate with accordance to Kafka ACL list.", 0);
                           enableKafkaSSL = 1;
                  }
           }
           if (strcmp(confName, "scheduler.kafkaCACertificate") == 0) {
		   kafkaCACertificate = malloc(strlen(confValue)+1);
		   if (kafkaCACertificate == NULL) {
			   fprintf(stderr, "Failed to allocate memory [kafkaCACertificate].\n");
			   writeLog("Failed to allocate memory [kafkaCACertificate]", 2);
			   return 2;
		   }
		   strcpy(kafkaCACertificate, trim(confValue));
		   if (enableKafkaExport > 0) {
                        writeLog("Kafka CACertificate not provided. Will try to connect to Kafka in plain text.", 1);
			enableKafkaSSL = 0;
                   }
	   }
           if (strcmp(confName, "scheduler.kafkaProducerCertificate") == 0) {
		   kafkaProducerCertificate = malloc(strlen(confValue)+1);
		   if (kafkaProducerCertificate == NULL) {
			   fprintf(stderr, "Failed to allocate memory [kafkaProducerCertificate].\n");
			   writeLog("Failed to allocate memory [kafkaProducerPertificate", 2);
			   return 2;
		   }
		   strcpy(kafkaProducerCertificate, trim(confValue));
		   if (enableKafkaExport > 0){
                   	writeLog("Certificate for Almond Kafka Producer functionality not provided. Will try to connect to Kafka in plain text.", 1);
                   	enableKafkaSSL = 0;
	   	}
	   }
           if (strcmp(confName, "scheduler.kafkaSSLKey") == 0) {
		   kafkaSSLKey = malloc(strlen(confValue)+1);
		   if (kafkaSSLKey == NULL) {
			   fprintf(stderr, "Failed to allocate memory [kafkaSSLKey].\n");
			   writeLog("Failed to allocate memory [kafkaSSLKey]", 2);
			   return 2;
		   }
		   strcpy(kafkaSSLKey, trim(confValue));
		   if (enableKafkaExport > 0) {
           		writeLog("No SSL key for Kafka producer certificate provided. Will try to connect to Kafka in plain text.", 1);
                   	enableKafkaSSL = 0;
		   }
	   }
...
void apiDryRun(int plugin_id) {
	...
	output.retString = malloc(pluginoutput_size);
	//memset(message, 0, sizeof message);
	message = malloc(apimessage_size * sizeof(char));
	if (message == NULL) {
		writeLog("Failed to allocate memory for api message", 1);
		return;
	}
        //pluginName = malloc(strlen(declarations[plugin_id].name)+1);
	pluginName = malloc(pluginitemname_size * sizeof(char));
	if (pluginName == NULL) {
        	fprintf(stderr, "Failed to allocate memory in apiDryRun.\n");
                writeLog("Failed to allocate memory [apiDryRun: pluginName", 2);
                return;
        }
	...
	if (socket_message == NULL) {
        	fprintf(stderr, "Failed to allocate memory for socket messages.\n");
                writeLog("Failed to allocate memory [apiDryRun:socket message]", 2);
                return;
	}
...
void apiRunPlugin(int plugin_id, int flags) {
	...
	if (pluginName == NULL) {
		fprintf(stderr, "Failed to allocate memory in apiRunPlugin.\n");
                writeLog("Failed to allocate memory [apiRunPlugin: pluginName]", 2);
                return;
        }
        //strcpy(pluginName, declarations[plugin_id].name);
	...
	while (threadIds[(short)plugin_id] > 0) {
		writeLog("Waiting for thread to finish...", 0);
		sleep(0.5);
	}
	...
	if (socket_message == NULL) {
		fprintf(stderr, "Failed to allocate memory in apiRunPlugin.\n");
                writeLog("Failed to allocate memory [apiRunPlugin: socket_message]", 2);
                return;
        }
	strcpy(socket_message, message);
	free(pluginName);
...
void runPluginArgs(int id, int aflags, int api_action) {
	...
	if (message == NULL) {
		...
		writeLog("Failed to allocate memory in [runPluginArgs:message].", 2);
		return;
	}
	newcmd = malloc(200);
	if (newcmd == NULL) {
		fprintf(stderr, "Failed to allocate memory in runPluginArgs.\n");
		writeLog("Failed to allocate memory [runPluginArgs: newcmd]", 2);
		return;
	}
	//command = malloc(strlen(declarations[id].command)+1);
	command = malloc(pluginitemcmd_size * sizeof(char));
	if (command == NULL) {
                fprintf(stderr, "Failed to allocate memory in runPluginArgs.\n");
                writeLog("Failed to allocate memory [runPluginArgs: command]", 2);
                return;
        }
	...
	if (pluginName == NULL) {
                fprintf(stderr, "Failed to allocate memory in runPluginArgs.\n");
                writeLog("Failed to allocate memory [runPluginArgs: pluginName]", 2);
                return;
        }
	...
	if (output.retString == NULL) {
		...
		writeLog("Failed to allocate memory [runPluginArgs:output.retString).", 2);
		return;
	}
	...
	fp = popen(newcmd, "r");
		...
		if (socket_message == NULL) {
                	fprintf(stderr, "Failed to allocate memory in runPluginArgs.\n");
                	writeLog("Failed to allocate memory [runPluginArgs: socketmessage]", 2);
                	return;
        	}
        	strcpy(socket_message, message);
		free(api_args);
	...
	if (socket_message == NULL) {
		fprintf(stderr, "Failed to allocate memory.\n");
		writeLog("Failed to allocate memory [runPluginArgs:socket_message]", 2);
		return;
	}
        strcpy(socket_message, message);
	free(api_args);
...
void apiReadData(int plugin_id, int flags) {
	...
	if (pluginName == NULL) {
		...
		writeLog("Failed to allocate memory [apiReadData:pluginName]", 2);
		return;
	}
        //strcpy(pluginName, declarations[plugin_id].name);
	pluginName = strdup(declarations[plugin_id].name);
	...
	if (socket_message == NULL) {
		fprintf(stderr, "Failed to allocate memory.\n");
		writeLog("Failed to allocate memory in [apiReadData:socket_message]", 2);
		return;
	}
	strcpy(socket_message, message);
	free(message);
...
void apiRunAndRead(int plugin_id, int flags) {
	...
	message = malloc(apimessage_size * sizeof(char));
	if (message == NULL) {
		writeLog("Could not allocate memory for apimessage.\n", 2);
		return;
	}
        //pluginName = malloc(strlen(declarations[plugin_id].name)+1);
	pluginName = (char *)malloc(pluginitemname_size * sizeof(char));
	if (pluginName == NULL) {
		fprintf(stderr, "Memory allocation failed.\n");
		writeLog("Failed to allocate memory [apiRunAndRead:pluginName]", 2);
		return;
	}
        strcpy(pluginName, declarations[plugin_id].name);
        removeChar(pluginName, '[');
	...
	if (socket_message == NULL) {
		fprintf(stderr, "Failed to allocate memory.\n");
		writeLog("Failed to allocate memory [apiRunAndRead:socket_message]", 2);
		return;
	}
	strcpy(socket_message, message);
	free(pluginName);
...
void apiReadFile(char *fileName, int type) {
	...
	f = fopen(fileName, "r");
		...
		if (message == NULL) {
			writeLog("Failed to allocate memory [apiReadFile:message]", 2);
			return;
		}
                if (message) {
		...
		if (socket_message == NULL) {
			fprintf(stderr,"Memory allocation failed.\n");
			writeLog("Failed to allocate memory [apiReadFile:socket_message]", 2);
			return;
		}
                strcpy(socket_message, message);
        }
		...
		if (socket_message == NULL) {
			writeLog("Failed to allocate memory in apiReadFile:err:socket_message", 2);
			return;
		}
...
void collectJsonData(int decLen){
	...
	//dataFName = (char *) malloc(filename_size * sizeof(char));
	strcpy(fileName, dataDir);
	strncat(fileName, &ch, 1);
	strcat(fileName, jsonFileName);
	...
	for (int i = 0; i < decLen; i++) {
		...
		if (pluginName == NULL) {
			fprintf(stderr, "Memory allocation failed.\n");
			writeLog("Failed to allocate memory [collectJsonData:pluginName]", 2);
			return;
		}
		//strcpy(pluginName, declarations[i].name);
		pluginName = strdup(declarations[i].name);
...
void collectMetrics(int decLen, int style) {
	...
	for (int i = 0; i < decLen; i++) {
		...
		if (pluginName == NULL) {
			fprintf(stderr, "Memory allocation failed.\n");
			writeLog("Memory allocation failed [collectMetrics:pluginName]", 2);
			return;
		}
       		//strcpy(pluginName, declarations[i].name);
		pluginName = strdup(declarations[i].name);
		...
		if (strchr(outputs[i].retString, '|') == NULL) {
			...
			if (style == 0)
				...
				serviceName = (char *)malloc(pluginitemdesc_size * sizeof(char));
				if (serviceName == NULL) {
					fprintf(stderr, "Failed to allocate memory.\n");
					writeLog("Failed to allocate memory [collectMetrics:serviceName]", 2);
					return;
				}
			...
			if (style == 0)
				...
				while (token != NULL) {
					...
					if (metricsToken == NULL) {
						writeLog("Failed to allocate memory [collectMetrics:metricsToken]", 2);
						return;
					}
					int do_cut = 0;
					...
					if (metricsName == NULL) {
						writeLog("Failed to allocate memory [collectMetrics:metricsName]", 2);
						return;
					}
                                        strcpy(metricsName, metricsToken);
					metricsValue = malloc(strlen(metricsName-index)+1);
					if (metricsValue == NULL) {
						writeLog("Failed to allocate memory [collectMetrics:metricsValue]", 2);
						return;
					}
...
void runPlugin(int storeIndex, int update) {
	...
	if (logPluginOutput == 1) {
		...
		o_info = malloc(o_info_size * sizeof(char));
		if (o_info == NULL) {
			writeLog("Could not allocate memory for variable 'o_info'.", 2);
		}
		if (update == 0)
			snprintf(o_info, o_info_size, "%s : %s", declarations[storeIndex].name, pluginReturnString);
		else
			snprintf(o_info, o_info_size, "%s : %s", update_declarations[storeIndex].name, pluginReturnString);
	...
	if (pluginResultToFile == 1) {
		...
		checkName = malloc(pluginitemname_size * sizeof(char));
		if (checkName == NULL) {
			writeLog("Failed to allocate memory [runPlugin:checkName].", 2);
			return;
		}
		/*if (update == 0)
	...
	if (enableKafkaExport == 1) {
		...
		if (pluginName == NULL) {
			fprintf(stderr, "Memory allocation failed.\n");
			writeLog("Failed to allocate memory [runPlugin:enableKafkaExport:pluginName]", 2);
			return;
		}
		...
		if (payload == NULL) {
			...
			writeLog("Failed to allocate memory [runPlugin:enableKafkaExport:payload]", 2);
			return;
		}
		//printf("Allocated payload: %d\n", count_bytes);
			...
			if (kafka_export_addons == KAFKA_EXPORT_TAG) {
				...
				int nKafkaId = kafka_start_id + storeIndex;
                                ...
                                char* kafka_id = malloc(length + 1);
                                snprintf(kafka_id, length+1, "%d", nKafkaId);
...
void copyPluginItem(PluginItem *dest, const PluginItem *src, int mode) {
	if (mode == 0) {
		if (src->name != NULL) {
			...
			writeLog("copyPluginItem[src->name] is empty. Do not copy.", 0);
		}
	}
	else if (mode == 2) {
		strncpy(dest->lastRunTimestamp, src->lastRunTimestamp,20);
...
void copyOutputItem(PluginOutput *dest, const PluginOutput *src) {
	...
	if (src->retString != NULL) {
		...
		writeLog("copyOutputItem source->retString is NULL", 1);
	}
}
...
int redeclarePluginDeclarations(int mode, int count) {
	...
	if (!update_declarations) {
		...
		writeLog("Error allocating memory [redeclarePluginDeclarations:update_declarations]", 2);
		abort();
		return 2;
	}
	for (int i = 0; i < count; i++) {
		update_declarations[i].name = (char *) malloc(pluginitemname_size * sizeof(char));
		update_declarations[i].description = (char *) malloc(pluginitemdesc_size * sizeof(char));
		update_declarations[i].command = (char *) malloc(pluginitemcmd_size * sizeof(char));
		if (update_declarations[i].name == NULL || update_declarations[i].description == NULL || update_declarations[i].command == NULL) {
			...
			writeLog("Error allocating memory [redeclarePluginDeclarations:update_declarations::items].", 2);
			abort();
			return 2;
		}
	}
	...
	if (!update_outputs){
		perror("Error allocating memory");
		writeLog("Error allocating memory [redeclarePluginDeclarations:update_outputs].", 2);
		abort();
		return 2;
	}
	for (int j = 0; j < count; j++) {
		update_outputs[j].retString = (char *) malloc(pluginoutput_size * sizeof(char));
		if (!update_outputs[j].retString) {
			fprintf(stderr, "Error allocating memory while redeclaring outputs.\n");
			writeLog("Error allocating memory [redeclarePluginDeclarations:update_outputs:returnString].", 2);
			abort();
			return 2;
		}
	}
	...
	switch(mode) {
		case 0:
				...
				for (int j = 0; j < decCount; j++) {
					if (strcmp(update_declarations[i].name, declarations[j].name) == 0) {
						...
						if (i == j) {
							...
							printf("DEBIG: Redeclare with new id.\n");
			...
			free(outputs);
                        declarations = NULL;
			declarations = (PluginItem *)malloc(sizeof(PluginItem) * count);
                        outputs = (PluginOutput *)malloc(sizeof(PluginOutput) * count);
                        if (!declarations) {
                                perror ("Error allocating memory");
                                writeLog("Error allocating memory [redeclarePluginDeclarations:redeclare_declaraions]", 2);
                                abort();
                                return 2;
                        }
                        if (!outputs) {
                                perror ("Error allocating memory");
                                writeLog("Error allocating memory [redeclarePluginDeclarations:redeclare_outputs]", 2);
                                abort();
                                return 2;
                        }
                        for (int i = 0; i < count; i++) {
                                declarations[i].name = (char *) malloc(pluginitemname_size * sizeof(char));
                                declarations[i].description = (char *) malloc(pluginitemdesc_size * sizeof(char));
                                declarations[i].command = (char *) malloc(pluginitemcmd_size * sizeof(char));
                                outputs[i].retString = (char *) malloc(pluginoutput_size * sizeof(char));
		...
		case 1:
			for (int i = 0; i < count; i++) {
				...
				for (int j = 0; j < decCount; j++) {
					if (strcmp(update_declarations[i].name, declarations[j].name) == 0) {
						...
						// BUG IS HERE DARN pointers
						/*update_outputs[i].retCode = outputs[j].retCode;
						update_outputs[i].prevRetCode = outputs[j].prevRetCode;
						memcpy(update_outputs[i].retString, outputs[j].retString, strlen(outputs[j].retString));*/
						//copyOutputItem(&update_outputs[i], &outputs[j]);
						//snprintf(update_outputs[i].retString, pluginoutput_size, "%s", outputs[j].retString);
						...
						update_declarations[i].active = declarations[j].active;
						update_declarations[i].id = i;*/
						//copyPluginItem(&update_declarations[i], &declarations[j], 0);
						found++;
						break;
					}
					else {
			...
			for (int z = 0; z < count; z++) {
				copyPluginItem(&declarations[z], &update_declarations[z], 0);
...
int hardReloadPlugins(int cnt) {
	...
	free_structures(decCount);
        ...
        free(declarations);
        declarations = NULL;
        declarations = (PluginItem *)malloc(sizeof(PluginItem) * cnt);
        outputs = (PluginOutput *)malloc(sizeof(PluginOutput) * cnt);
        if (!declarations) {
        	perror ("Error allocating memory");
       		writeLog("Error allocating memory [redeclarePluginDeclarations:redeclare_declaraions]", 2);
        	abort();
       ...
       if (loadPluginDeclarations(pluginDeclarationFile, 0) == 0) {
	       writeLog("Plugin file reloaded.", 0);
       }
       else {
	       writeLog("Error reloading plugins file.", 2);
	       sig_handler(SIGSTOP);
       }
       decCount = cnt;
       initScheduler(cnt, 1000);
       quick_start = qsv;
       writeLog("Hard reload of plugins done. Scheduler restarted.", 0);
...
int updatePluginDeclarations() {
	...
	item.name = (char *)malloc(pluginitemname_size * sizeof(char));
	item.description = (char *)malloc(pluginitemdesc_size * sizeof(char));
	item.command = (char *)malloc(pluginitemcmd_size * sizeof(char));
...
void initScheduler(int numOfP, int msSleep) {
	...
	if (local_api > 0) {
		if (socket_is_ready == 1) {
			writeLog("Socket is already happy.", 0);
			return;
		}
	...
	while (i > 0) {
		...
		if (enableClearDataCache != 0) {
			...
			if (seconds > tnextClearDataCache) {
                                writeLog("ClearDataCash is ready", 0);
				clearDataCache();
...
int isConstantsEnabled () {
	...
	file = fopen("/etc/almond/constants.conf", "r");
	if (file == NULL) {
		printf("No constants file will be used.\n");
		writeLog("No constants.conf file was found.", 1);
		return 0;
	}
	while (fgets(line, sizeof(line), file)) {
		if (strstr(line, searchString)) {
			writeLog("Constants file is enabled.", 0);
			return 1;
			break;
		}
	}
...
int main() {
	...
	initConstants();
	writeLog("Starting almond (0.8.0)...", 0);
	...
	if (isConstantsEnabled() > 0) {	
		if (getConstants() > 0) {
			writeLog("Memory initialization failed. Exiting.", 2);
			return 2;
		}
        }
	...
	declarations = (PluginItem *)malloc(sizeof(PluginItem) * decCount);
	if (!declarations) {
		perror ("Error allocating memory");
	...
	for (int i = 0; i < decCount; i++) {
		declarations[i].name = malloc(pluginitemname_size);
		if (declarations[i].name == NULL) {
			fprintf(stderr, "Memory allocation failed.\n");
			writeLog("Failed to allocate declarations.", 2);
			exit(2);
		}
		declarations[i].description = malloc(pluginitemdesc_size);
		if (declarations[i].description == NULL){
			fprintf(stderr, "Memory allocation failed.\n");
                        writeLog("Failed to allocate declarations.", 2);
                        exit(2);
		}
		declarations[i].command = malloc(pluginitemcmd_size);
	...
	for (int i = 0; i < decCount; i++) {
		outputs[i].retString = malloc(pluginoutput_size);
	...
	threadIds = (unsigned short*)malloc(decCount * sizeof(unsigned short));
	for (int i = 0; i < decCount; i++) {
		threadIds[i] = 0;
	}
