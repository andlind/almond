...
void rescheduleChecks() {
	size_t n = (size_t)decCount;
        writeLog("Schedule new execution times.", 0, 0);
	checkSchedulerCount();
...
int initTimeScheduler(bool update_values) {
	if (decCount == 0) {
		scheduler = NULL;
	...
	if (update_values) {
		Scheduler *new_sched = calloc((size_t)decCount, sizeof(Scheduler));
		if (!new_sched) {
			perror("calloc");
			writeLog("Error allocating memory while updating scheduler [initTimeScheduler]", 2, 0);
			return 2;
		}
		for (int i = 0; i < decCount; ++i) {
			if (g_plugins && g_plugins[i]) {
				new_sched[i].id = g_plugins[i]->id;
				new_sched[i].timestamp = g_plugins[i]->nextRun;
			}
			else {
				new_sched[i].id = i;
				new_sched[i].timestamp = (time_t)0;
			}
		}
		free(scheduler);
		scheduler = new_sched;
...
void runPluginThreads(int loopVal){
	...
	if (timeScheduler) {
			...
			//printScheduler(scheduler, decCount);
			//printf("DEBUG: End debug scheduler.\n");
...
void initialLogging() {
	...
	if (!fptr) {
        ...
        writeLog("Starting almond (0.9.21)...", 0, 1);
}
