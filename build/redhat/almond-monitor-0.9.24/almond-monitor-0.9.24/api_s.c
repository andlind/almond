...
void apiMonitorHardItem(int plugin_id) {
        char* message = NULL;
        char retString[2280] = {0};
        char ch = '/';
        //PluginOutput output = {0};
        //char rCode[12];
	PluginOutput *output;
        int rc = 0;

	//printf("DEBUG: Entering apiMonitorHardItem\n");
    	//fflush(stdout); // FORCE THE PRINT TO APPEAR
	...
	memset(message, 0, apimessage_size);
        /*strcpy(message,"{\n     \"plugin\":\"");
	if (g_plugins[plugin_id] && g_plugins[plugin_id]->description) {
        	strcat(message, g_plugins[plugin_id]->description);
	}
        strcat(message, "\",\n");
        strcat(message, "     \"output\":\"");*/
        // Now run plugin_id and get output and rCode
	//printf("DEBUG: Starting plugin execution\n");
    	//fflush(stdout);
	...
	//printf("DEBUG: Calling tracked_popen with cmd: %s\n", pluginCommand);
	//fflush(stdout);
        TrackedPopen tp = tracked_popen(pluginCommand);
	//printf("DEBUG: tracked_popen returned fp: %p\n", (void*)tp.fp);
	//fflush(stdout);
        if (tp.fp == NULL) {
		...
		//printf("DEBUG: Entering fgets loop\n");
		//fflush(stdout);
        ...
        if (rc > 0)
        {
                ...
                output->retCode = rc;
	//printf("DEBUG: Plugin finished. rc=%d, raw output length=%zu\n", rc, strlen(retString));
	//fflush(stdout);
	// --- TEST 1: Is it the memset? ---
        //printf("DEBUG: Attempting memset...\n");
        //fflush(stdout);
	memset(output->retString, 0, pluginoutput_size);
	// --- TEST 2: Is it the message pointer? ---
        //printf("DEBUG: Checking message pointer...\n");
        //fflush(stdout);
        //if (message == NULL) {
        //    printf("DEBUG: message is NULL!\n"); fflush(stdout);
        //} else {
        //    printf("DEBUG: message is at %p\n", (void*)message); fflush(stdout);
        //}
	char* checked_output = trim(retString);
	//char* checked_output = retString; //USE RAW
	//printf("DEBUG: Attempting snprintf...\n");
        //fflush(stdout);
        //snprintf(output->retString, pluginoutput_size, "%s", checked_output);
        //printf("DEBUG: Reached Space Check!\n");
        //fflush(stdout);
	//memset(output.retString, 0, sizeof(output.retString));
	//printf("DEBUG: Trimmed output pointer: %p\n", (void*)checked_output);
	//fflush(stdout);
	if (checked_output) {
                snprintf(output->retString, pluginoutput_size, "%s", checked_output);
		//strncat(message, output.retString, apimessage_size - strlen(message) - 1);
        }
	//printf("DEBUG: Space check - message_len: %zu, limit: %ld\n", strlen(message), apimessage_size);
	//fflush(stdout);
	//int written = snprintf(message, apimessage_size,
        //"{\n     \"plugin\":\"%s\",\n     \"output\":\"%s\",\n     \"returncode\":%d\n}\n",
        //g_plugins[plugin_id]->description,
        //output->retString,
        //output->retCode);

    	//if (written >= apimessage_size) {
        //writeLog("Warning: API message truncated due to buffer size", 1, 0);
    	//}
	...
	//strncat(message, output.retString, apimessage_size - strlen(message) - 1);
        //strncat(message, "\",\n      \"returncode\":", apimessage_size - strlen(message) - 1);
	//strncat(message, "\",\n", apimessage_size - strlen(message) - 1);
        //strncat(message, "      \"returncode\":", apimessage_size - strlen(message) - 1);
        //sprintf(rCode, "%d", output.retCode);
        //strncat(message, trim(rCode), apimessage_size - strlen(message) - 1);
        //strncat(message, "\n}\n", apimessage_size - strlen(message) - 1);
