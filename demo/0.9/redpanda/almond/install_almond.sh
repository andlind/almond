...
#cd /redpanda/almond
docker cp almond-0.9.30 redpanda-10:/root
...
docker exec -u 0 redpanda-10 /bin/sh -c "cd /root/almond-0.9.30 && autoreconf -fi && ./install_almond.sh"
...
docker cp ../../alpine/libjwt/libjwt.so.2 redpanda-console-10:/usr/lib/libjwt.so.2
docker cp ../../alpine/libjwt/libjwt.so.14 redpanda-console-10:/usr/lib/libjwt.so.14
...
docker exec -u 0 redpanda-console-10 tar xfvz /tmp/almond-0.9.23.alpine.aarch64.tar.gz 
echo "Installing precompiled Almond"
docker exec -u 0 redpanda-console-10 /bin/sh -c "cd almond-0.9.23.alpine.aarch64 && ./install.sh"
echo "Installation finished"
echo "Start Almond"
