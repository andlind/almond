...
docker cp ../../alpine/almond/almond-0.9.9.alpine.aarch64.tar.gz almond_grafana:/tmp
...
docker exec -u 0 almond_grafana tar xfvz /tmp/almond-0.9.9.alpine.aarch64.tar.gz
echo "Installing precompiled Almond"
docker exec -u 0 almond_grafana /bin/bash -c "pwd && cd almond-0.9.9.alpine.aarch64 && ./install.sh"
