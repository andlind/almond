...
void startApiSocket() {
	...
	//pthread_setspecific(thread_id, "API Connection Listener");
        pthread_setname_np(thread_id, "API Conn List");
...
void* pluginExeThread(void* data) {
	...
	//threadIds[(short)storeIndex] = 1;
      	threadIds[(short)storeIndex] = (pthread_t)(uintptr_t)1;
	...
	pthread_mutex_unlock(&mtx);
        ...
        threadIds[(short)storeIndex] = (pthread_t)(uintptr_t)0;
	pthread_exit(NULL);
	total_threads_run++;
}
...
int hardReloadPlugins(int cnt) {
	...
	if (threadIds != NULL) {
 	...
 	threadIds = (pthread_t *)malloc((size_t)MAX_PLUGINS * sizeof(pthread_t));
...
int loadPlugins() {
	decCount = countDeclarations(pluginDeclarationFile);
        ...
        for (int i = 0; i < decCount; i++) {
                //threadIds[i] = 0;
                threadIds[i] = (pthread_t)(uintptr_t)0;
        }
