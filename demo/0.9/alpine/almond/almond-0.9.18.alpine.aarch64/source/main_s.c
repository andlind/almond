...
int values[50];
unsigned short *threadIdsEx = NULL;
...
void startApiSocket() {
	...
	pthread_key_create(&thread_key, NULL); // Create a key for thread-specific data
	pthread_setspecific(thread_key, "API Connection Listener");
	printf("New thread accepting socket created.\n");
        snprintf(infostr, infostr_size, "Created new thread (%p) listening for connections on port %d \n", (void *)thread_id, local_port);
        writeLog(trim(infostr), 0, 0);
	pthread_mutex_lock(&mtx);
	thread_counter++;
	pthread_mutex_unlock(&mtx);
...
void apiRunPlugin(int plugin_id, int flags) {
	...
	while (threadIdsEx[(short)plugin_id] > 0) {
...
void runPluginArgs(int id, int aflags, int api_action) {
	...
	if (newcmd == NULL) {
		...
		memset(newcmd, '\0', 200);
	command = malloc((size_t)(pluginitemcmd_size + 1) * sizeof(char) +1);
	if (command == NULL) {
                fprintf(stderr, "Failed to allocate memory in runPluginArgs.\n");
		...
		memset(command, '\0', (size_t)(pluginitemcmd_size + 1) * sizeof(char) + 1);
	pluginName = malloc((size_t)(pluginitemname_size + 1) * sizeof(char));
	if (pluginName == NULL) {
                fprintf(stderr, "Failed to allocate memory in runPluginArgs.\n");
...
void* pluginExeThread(void* data) {
	...
	pthread_mutex_lock(&mtx);
	threadIdsEx[(short)storeIndex] = 1;
	...
	thread_counter--;
	pthread_mutex_unlock(&mtx);
        threadIdsEx[(short)storeIndex] = 0;
	pthread_exit(NULL);
}
...
void startPluginThread(int plugin_id) {
	...
	if(rc) {
		...
		snprintf(infostr, infostr_size, "Created new thread (%p) for plugin %s\n", (void *)thread_id, g_plugins[plugin_id]->name);
		writeLog(trim(infostr), 0, 0);
		pthread_mutex_lock(&mtx);
		thread_counter++;
		pthread_mutex_unlock(&mtx);
		...
		if (t > g_plugins[i]->nextRun)
		{
			rc = pthread_create(&thread_id, NULL, pluginExeThread, (void *)j);
				...
				writeLog(trim(infostr), 2, 0);
                   		...
                   		snprintf(infostr, infostr_size, "Created new thread (%p) for plugin %s\n", (void *)thread_id, g_plugins[i]->name);
				writeLog(trim(infostr), 0, 0);
				pthread_mutex_lock(&mtx);
				thread_counter++;
				pthread_mutex_unlock(&mtx);
...
void executeGardener() {
	...
	pthread_key_t thread_key;
	pthread_key_create(&thread_key, NULL); // Create a key for thread-specific data
	pthread_setspecific(thread_key, "Gardener worker");
	snprintf(infostr, infostr_size, "Created new thread (%p) truncating metrics logs (gardener) \n", (void *)thread_id);
        writeLog(trim(infostr), 0, 0);
	pthread_mutex_lock(&mtx);
	thread_counter++;
	pthread_mutex_unlock(&mtx);
...
void clearDataCache() {
	...
	rc = pthread_create(&thread_id, NULL, clearDataCacheThread, "clearDataCache 1");
		...
		pthread_key_t thread_key;
		pthread_key_create(&thread_key, NULL); // Create a key for thread-specific data
		pthread_setspecific(thread_key, "DataClearCache");
                snprintf(infostr, infostr_size, "Created new thread (%p) clearing old data files (clearDataCache) \n", (void *)thread_id);
                writeLog(trim(infostr), 0, 0);
		pthread_mutex_lock(&mtx);
		thread_counter++;
		pthread_mutex_unlock(&mtx);
...
void initialLogging() {
	char lfin[28] = "/var/log/almond/almond.log";
        ...
        fptr = fopen(lfin, "a");
	if (!fptr) {
		perror("Failed to open log file '/var/log/almond/almond.log'");
		exit(EXIT_FAILURE);
	}
        fprintf(fptr, "\n");
        ...
        writeLog("Starting almond (0.9.18)...", 0, 1);
}
...
int main(int argc, char* argv[]) {
	...
	if (check_plugin_conf_file(pluginDeclarationFile) != 0) {
        ...
        threadIdsEx = (unsigned short*)malloc((size_t)MAX_PLUGINS * sizeof(unsigned short));
	threadIds = malloc(sizeof(pthread_t) * MAX_PLUGINS);
    	memset(threadIdsEx, 0, MAX_PLUGINS * sizeof(unsigned short));
    	for (int i = 0; i < decCount; i++) {
        	threadIdsEx[i] = 0;
    	}
    	//thread_counter++;
