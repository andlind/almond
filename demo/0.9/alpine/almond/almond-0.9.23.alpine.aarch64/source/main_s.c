...
int values[50];
unsigned char *threadRunning = NULL;
...
void startApiSocket() {
	...
	pthread_setspecific(key, "API Connection Listener");
...
void executeGardener() {
	...
	pthread_key_create(&key, NULL);
	pthread_setspecific(key, "Gardener worker");
...
void clearDataCache() {
	...
	rc = pthread_create(&thread_id, NULL, clearDataCacheThread, "clearDataCache 1");
		...
		pthread_key_create(&key, NULL);
		pthread_setspecific(key, "DataClearCache");
...
void initialLogging() {
	...
	if (!fptr) {
        ...
        writeLog("Starting almond (0.9.23)...", 0, 1);
}
...
int main(int argc, char* argv[]) {
	...
	//threadIds = malloc(MAX_PLUGINS) * sizeof(pthread_t));
	threadRunning = calloc(MAX_PLUGINS, sizeof(unsigned char));
    	//memset(threadIds, 0, MAX_PLUGINS * sizeof(unsigned short));
	//memset(threadIds, 0, MAX_PLUGINS * sizeof(pthread_t));
	decCount = countDeclarations(pluginDeclarationFile);
    	/*for (int i = 0; i < decCount; i++) {
        	pthread_join(threadIds[i], NULL);
    	}*/
