...
nohup docker run --rm  --net almond-redpanda-quickstart_almond-demo --name redis_demo -d -v ~/almond_demo/data:/opt/almond/data -p 6379:6379 redisdemo
echo "Starting custom (backend) app"
docker buildx build --platform linux/amd64 -t custapplication custapp
nohup docker run --rm --platform linux/amd64 --cap-add=SYS_TIME --cap-add=SYS_NICE --cap-add=SYS_PTRACE --net almond-redpanda-quickstart_almond-demo --name customapp_demo -d -v ~/almond_demo/data:/opt/almond/data -p 8076:8099 -p 9909:9909 custapplication
